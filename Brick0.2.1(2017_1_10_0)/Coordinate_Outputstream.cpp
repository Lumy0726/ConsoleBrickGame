...
void C_screen::Show_opt_kr(unsigned Time){
	...
	while (Print){
		...
		if (Start_time.Get_time() >= Time){ Print = false; }
	}
...
void C_screen_color::Reverse_color(){
	Color = (Color << 4 & 0xf0 | Color >> 4 & 0x0f) & 0xff;
}
...
void C_screen_color::Show_opt_kr(unsigned Time){
	...
	while (Print){
		if ((Screen_buf[Y][X] != Screen[Y][X]) ||
			...
			cout << Screen[Y][X];//출력.
			SetConsoleTextAttribute(H, Screen_col[Y][X + 1] & 0xff);
			cout << Screen[Y][X + 1];
			Screen_buf[Y][X] = Screen[Y][X];//저장.
			Screen_buf[Y][X + 1] = Screen[Y][X + 1];//저장.
...
int random(unsigned N){
	...
	return unsigned((rand() * (RAND_MAX + 1) + rand()) % ((0 < N&&N <= (RAND_MAX + 1) * (RAND_MAX + 1)) ? N : (RAND_MAX + 1) * (RAND_MAX + 1)));
}
