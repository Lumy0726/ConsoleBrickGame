...
//protected:
	...
	void Arrange_ball(unsigned Ball_num);//영영 Hitting_zone으로 들어오지 못하는 공을 정리함.
...
int main(){
	C_board Test;
	Test.PLAY();
	for (int Temp1 = 0; Temp1 < 50; Temp1++){
		cout << random(10);
	}
 	return 0;
}
	...
	for (Temp1 = 0; Temp1 < Ball_rec.Num() - 1; Temp1++){//공 그림자.(반사.)
		...
		if (Ball_rec.Read(Temp1).X == -1){ ; }//출력 필요 없음.(Reflector반사표식).
			...
			if (Ball_rec.Read(Temp1).Vector_X() != Ball_rec.Read(Temp1 + 1).Vector_X()){
				...
				if (X_ref == Ball_rec.Read(Temp1 + 1).X){ X_ref = -1; }//반사점과 무반사경우의 이동위치가 같은 경우.
			}
			else { X_ref = -1; }
			if (Ball_rec.Read(Temp1).Vector_Y() != Ball_rec.Read(Temp1 + 1).Vector_Y()){
				...
				if (Y_ref == Ball_rec.Read(Temp1 + 1).Y){ Y_ref = -1; }//반사점과 무반사경우의 이동위치가 같은 경우.
			}
			else { Y_ref = -1; }
			...
			if (X_ref == -1 && Y_ref == -1){//백터의 방향은 바뀌었으나 무반사로 출력됨. 직접 반사표식점 출력 필요함.
				//(이 코드가 존재하는 이유는, 벽돌 반사가 아닌 공의 꺽인점의 경우에는, 반사라 가정하고 반사점을 넣으면 반사위치가 INTERNAL_RESOLUTION의 배수가 아닐수도 있기 때문에 Print_line에서 오류 발생.)
				Screen.Setcolor(C_screen_color::RED, BACK_COLOR);
				Screen << Coord(
					((Ball_rec.Read(Temp1 + 1).Vector_X() >= 0) ? FLOOR(Ball_rec.Read(Temp1 + 1).X, INTERNAL_RESOLUTION) : CEIL(Ball_rec.Read(Temp1 + 1).X, INTERNAL_RESOLUTION) - INTERNAL_RESOLUTION) * 2 / INTERNAL_RESOLUTION,
					((Ball_rec.Read(Temp1 + 1).Vector_Y() >= 0) ? FLOOR(Ball_rec.Read(Temp1 + 1).Y, INTERNAL_RESOLUTION) : CEIL(Ball_rec.Read(Temp1 + 1).Y, INTERNAL_RESOLUTION) - INTERNAL_RESOLUTION) / INTERNAL_RESOLUTION
					) << Str;
			}
		}
	}
		...
		if (Board[Y][X].Health == 0){
			for (Temp1 = X1; Temp1 <= X2; Temp1++){
				for (Temp2 = Y1; Temp2 <= Y2; Temp2++){
					Screen << Coord(Temp1 * 2, Temp2) << "  ";
				}
			}
		}
		else if (Board[Y][X].Health > 0){
			//블럭 테두리.
			Screen << Coord(X1 * 2, Y1) << "■";
			Screen << Coord(X1 * 2, Y2) << "■";
			Screen << Coord(X2 * 2, Y1) << "■";
			Screen << Coord(X2 * 2, Y2) << "■";
			for (Temp1 = 0; Temp1 < BRICK_SIZE - 2; Temp1++){
				Screen << Coord(X1 * 2, Y1 + Temp1 + 1) << "■";
				Screen << Coord(X2 * 2, Y1 + Temp1 + 1) << "■";
				Screen << Coord((X1 + Temp1 + 1) * 2, Y1) << "■";
				Screen << Coord((X1 + Temp1 + 1) * 2, Y2) << "■";
			}
			//블럭 내부.
			if (Board[Y][X].State == C_brick::NORMAL){
				for (Temp1 = 0; Temp1 < BRICK_SIZE - 2; Temp1++){
					for (Temp2 = 0; Temp2 < BRICK_SIZE - 2; Temp2++){
						Screen << Coord((X1 + Temp1 + 1) * 2, Y1 + Temp2 + 1) << "  ";
					}
				}
			}
...
void C_board::Arrange_ball(unsigned Ball_num){
	if (Ball_num >= Balls.Num()){ return; }
