...
public:
	...
	void Arrange_ball_in_brick(unsigned Ball_num, Direction Way);//공이 벽돌에 끼는 경우를 정리.
...
C_board::C_board(unsigned Base_size, unsigned Field_size, unsigned Brick_height)
	...
	cout << "공이 반사판에 닿지 않고 중앙의 빨간 영역을 완전히 통과하면, 공의 속도가 최저로 떨어지며, 이미 최저였던 경우에는 공이 사라집니다." << endl;
	...
	while (1){
		...
		while (Check){//한 프레임.
			...
			if (Playbrick_tick == Playbrick_speed_tick){
				...
				if (Play_brick()){ Check = false; }//게임오버.
			}
			else { Playbrick_tick++; }
#ifdef MYDEBUG
			...
			if (Lag_check < 0){//시간 지연 발생.
				...
				for (Temp1 = 0; Temp1 < BRICK_HEIGHT*BRICK_SIZE * 2; Temp1 += 2){
...
void C_board::Arrange_ball_in_brick(unsigned Ball_num, Direction Way){//공이 벽돌에 끼는 경우를 정리.
	if (Ball_num >= Balls.Num()){ return; }
	C_ball& Ball = *Balls.Read_editable(Ball_num);
	C_ball::C_ball_vector * Lastball;
	int Brick_X, Brick_Y;
...
void C_board::Brick_hit(unsigned X, unsigned Y, unsigned Speed){
	int Temp1;
	static const unsigned int Damageup_speed = (2 * C_board::BRICK_SIZE * C_board::INTERNAL_RESOLUTION / 5) / VECTOR_LENGTH;//공의 최대 속도를 5등분.
	"";//벽돌속성추가.
