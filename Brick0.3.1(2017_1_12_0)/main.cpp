...
protected:
	...
	enum{
		...
		SHIFT_BLOCK_TICK = 100,//쉬프트 연속이 금지되는 틱 수.
	};
	...
	bool Check_unbreak_brick(unsigned X, unsigned Y, T_direction Callbrick = UR);
...
void C_board::Game_init(){
	...
	Level = 1; Score = 0, Play_brick_way = U, Levelup_tick = 1, Shift_on = false, Shift_block_tick = 0;
	//공 초기화.
...
void C_board::PLAY(bool Loadstate){
	...
	while (1){
		...
		while (Check){//한 프레임.
...
#endif
			//Key Input.
			if (Shift_block_tick != 0){ Shift_block_tick--; }
			//쉬프트를 다시 누르기 시작할수 있는 주기는 SHIFT_BLOCK_TICK * OUTPUT_TICK이며, 그 시간의 5분의 1동안만 쉬프트 누루고 있기 가능.
			if (C_keyboard::Check_state(C_keyboard::LSHIFT)){//현재 쉬프트 누른 상태.
				if (Shift_on){
					if (Shift_block_tick <= SHIFT_BLOCK_TICK * 4 / 5){
						Shift_on = false;
					}
				}
				else {//쉬프트 꺼져있었음.
					if (Shift_block_tick == 0){//쉬프트 누르기 가능.
						Shift_block_tick = SHIFT_BLOCK_TICK;
						Shift_on = true;
					}
				}
			}
			...
			if (C_keyboard::Check_state(C_keyboard::U)){
				while (Balls.Num() > 2){
					Balls.Out();
				}
				while (Balls.Num() < 2){
					Balls.In(C_ball(INTERNAL_RESOLUTION / 2, INTERNAL_RESOLUTION / 2));
				}
			}
#endif
...
void C_board::Print_ball(unsigned Ball_num, bool Print){
	...
	Speed = int(CEIL(Balls.Read(Ball_num).Speed_movenum / Balls.Read(Ball_num).Speed_tick, Damageup_speed) / Damageup_speed);
	if (Speed > 5){
...
void C_board::Print_Reflector(bool Print){
	...
	switch (Shift_block_tick * 5 / SHIFT_BLOCK_TICK){
	case 0:
...
bool C_board::Check_unbreak_brick(unsigned X, unsigned Y, T_direction Callbrick){//사라질수 있으면 true반환.
...
void C_board::Move_reflector(T_direction Input, unsigned Speed){
	...
	for (Loop1 = 0; Loop1 < (int)Balls.Num(); Loop1++){
		...
		if (Ball_speedup){//공이 끼인적이 있는 경우.
			Balls.Read_editable(Loop1)->Speed_edit(Speed, Max_movenum);//속도 증가.
...
void C_board::Arrange_ball_zoneout(unsigned Ball_num){//영영 Hitting_zone으로 들어오지 못하는 공을 정리함.
	...
	int H1 = (BRICK_HEIGHT + 1) * BRICK_SIZE*INTERNAL_RESOLUTION + 1;
	int H2 = (BOARD_SIZE - BRICK_HEIGHT - 1) * BRICK_SIZE*INTERNAL_RESOLUTION - 1;
	bool Check = false;
...
void C_board::Brick_add(unsigned X, unsigned Y){
	...
	if (Balls.Num() != 0){
		if (random(Balls.Num() * 20) == 0){//제일 없어서는 안되는 속성. 마지막으로 검사함으로써 이전의 속성을 무시하고 설정함.
...
unsigned C_board::Dump_size() const{
	return sizeof(unsigned) + sizeof(int) * 9/*enum값들*/ + sizeof(unsigned) * 5 + sizeof(C_screen_color::T_color) + sizeof(C_brick) * BOARD_SIZE * BOARD_SIZE + Balls.Dump_size()
		+ sizeof(bool) + sizeof(unsigned) * 8 + sizeof(T_direction) + sizeof(T_reflector);
}
...
bool C_board::Restore(char * Ptr){
	...
	Ptr += sizeof(int);
	if (*(int*)Ptr != SHIFT_BLOCK_TICK){ return false; }
	Ptr += sizeof(int);
