		...
		for (Temp1 = 0; Temp1 < int(BASE_SIZE + 2 * FIELD_SIZE); Temp1++){//한 줄.
			...
			for (Temp2 = 0; Temp2 < (int)BOARD_SIZE; Temp2++){//줄 내부 처리.
				if (Board[Temp2][X].Health == 0){//블럭 존재 안함.-->밀리기 가능.
					...
					Print_brick(X, 0);
					//공이 포함되면 밀기.
					for (Temp3 = 0; Temp3 < (int)Balls.size(); Temp3++){//하나의 공.
						for (Temp4 = 0; Temp4 < (int)Balls.at(Temp3).Ball_rec.Num(); Temp4++){//공의 그림자중 하나.
							Ball = Balls.at(Temp3).Ball_rec.Read_editable(Temp4);
