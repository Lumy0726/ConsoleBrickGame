...
C_board::C_board(unsigned Base_size, unsigned Field_size, unsigned Brick_height)
	...
	cout << "반사판 움직이기: 방향키.(A를 같이누르면 3배 가속.)" << endl;
	cout << "반사판 돌리기: W,D (A를 같이누르면 2배 가속.)" << endl;
...
void C_board::PLAY(bool Loadstate){
	...
	while (1){
		...
		while (Check){//한 프레임.
			...
			if (C_keyboard::Check_state(C_keyboard::A)){//현재 부스트 누른 상태.
				if (Boost_on){
...
bool C_board::Restore(char * Ptr){
	...
	Ptr += sizeof(int);
	if (*(int*)Ptr != Boost_block_tick){ return false; }
	Ptr += sizeof(int);
