...
void C_board::PLAY(){
	...
	while (1){
		...
		while (Check){//한 프레임.
			...
			if (Time.Get_time() >= OUTPUT_FRAME){ Lag_check = true; }//속도 지연 발생.
			if (Lag_check){
				Screen.Setcolor(C_screen_color::BLACK, C_screen_color::RED);
			}
			else {
				Screen.Setcolor(C_screen_color::BLACK, C_screen_color::GREEN);
			}
			Screen << Coord(0, Screen.GetH()) << "        ";
			while (Time.Get_time() < OUTPUT_FRAME){ ; }
		}
		//게임 오버.
...
void C_board::Print_ball(unsigned Ball_num, bool Print){
	...
	//현재 공.
	Screen.Setcolor(C_screen_color::RED, BACK_COLOR);
	Screen << Coord((FLOOR(Ball_rec.Read(Temp1).X, INTERNAL_RESOLUTION) / INTERNAL_RESOLUTION) * 2, FLOOR(Ball_rec.Read(Temp1).Y, INTERNAL_RESOLUTION) / INTERNAL_RESOLUTION) << Str;
}
